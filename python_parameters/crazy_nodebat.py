...
class CrazyNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('Ncell', [1,1] ) # series, parallel
        # --------------------
        ...
        self.vcell = 4.2
        self.vcellmin = 3.5
        self.vbat_tot = self.vcell*self.Ncell[0]
        self.Ccell = self.Cbat/self.Ncell[1]
        self.vbat = self.vbat_tot
        self.E_cell = 0.0
    ...
    def timer_callback(self):
        ...
        self.get_logger().info(f"Battery voltage: {self.vbat}")  
        self.Priority[len(self.Priority) - 1] = -0.1 + (self.vbat - self.vcell)*self.Ncell[0]/(self.vcellmin - self.vcell*self.Ncell[0])*1.1
        i = 0
        if self.Priority[len(self.Priority) - 1] < 0.7 or all(Prio < 0.2 for Prio in self.Priority[:-1]):
    ...
    def battery_model(self):
        ...
        # Initialize t_last the first time
        if self.t_last.nanoseconds == 0:
            self.t_last = now
        ...
        # -----------------------------
        MAX_DT = 1/self.control_rate

        if dt_raw < 0.0 or dt_raw > MAX_DT:
            self.get_logger().warn(
                f"⚠️ Abnormal time interval detected! dt = {dt_raw:.6f} s (expected < {MAX_DT:.3f} s). "
                "Possible clock jump or timer delay."
            )

        # Clamp dt
        dt_s = max(0.0, min(dt_raw, MAX_DT))
